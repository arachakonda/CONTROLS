...
class Controller:
    ...
    def __init__(self):
        ...
        self.m_cap_ = 1.45
        self.eeta_p_ = np.array([4.0,4.0,1.0])
        self.Phi = np.array([4.0,4.0,3.0])
        self.v = 1.0
        self.Fp_ = 0.5
    ...
    def multiDoFCb(self, msg):
        ...
        if(self.magnet_ON):
            if (pt.transforms[0].translation.x < -0.48 and pt.transforms[0].translation.x > -0.52) \
            and (pt.transforms[0].translation.y > -0.52 and pt.transforms[0].translation.y < -0.48 ):            
    ...
    def th_des(self):
        ...
        print(errPos)
        up_cap = self.m_cap_*(self.gravity + self.acceleration - np.multiply(self.Phi, errVel))
        kp = self.eeta_p_ + self.Fp_*(self.gravity + self.acceleration - np.multiply(self.Phi, errVel))

        # Rho = self.Kp0 + self.Kp1*errPos

        delTau = np.zeros(3)
        delTau[0] = kp[0]*self.sigmoid(sv[0],self.v)
        delTau[1] = kp[1]*self.sigmoid(sv[1],self.v)
        delTau[2] = kp[2]*self.sigmoid(sv[2],self.v)

        des_th = up_cap - delTau
