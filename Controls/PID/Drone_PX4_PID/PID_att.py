...
class Controller:
    ...
    def __init__(self):
        ...
        # Gains
        self.Kpos = np.array([-4, -4, -5])
        self.Kvel = np.array([-3, -3, -3])
        self.Kint = np.array([-0.1, -0.1, -1.0])
        self.norm_thrust_const = 0.045
        self.max_acc = 4
        self.max_throttle = 0.99
        self.gravity = np.array([0, 0, 14.8])
    ...
    def multiDoFCb(self, msg):
        ...
        self.yaw_angle.data = np.arctan2(self.sp.pose.position.y, self.sp.pose.position.x)
        if(self.magnet_ON):
    ...
    def a_des(self):
        ...
        return (des_a + self.gravity) , curor
    ...
    def geo_con(self):
        des_a , curorien = self.a_des()
        yaw_deg = float(self.yaw_angle.data)
        yaw_rad = (yaw_deg*np.pi)/(180)    
        r_des = self.acc2quat(des_a, self.start_yaw*yaw_deg)
