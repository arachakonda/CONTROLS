...
void mycontroller(const mjModel* m, mjData* d)
{
  ...
  if(t_sec <=40) {
      ...
      //only qdd_d terms are needed saparately as the multiplication with it occurs in STEP 3
      ...
      // get the (Kd . e_qd) term ------------- STEP 1
      mju_scl(kde_qd, e_qd, kv, 3);
      // get the (Kp . e_q) term -------------- STEP 2
      mju_scl(kpe_q, e_q, kp, 3);
      // get the (M . qdd_d) term ------------- STEP 3
      mju_mulMatVec(mqdd_d , dense_M, qdd_d, 3, 3);
      // get the (M . Kd . e_qd) term --------- STEP 4
      mju_mulMatVec(mkde_qd, dense_M, kde_qd, 3, 3);
      // get the (M . Kp . e_q) term ---------- STEP 5
